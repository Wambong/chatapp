let input_message = $('#input-message'); // Updated ID for text input
let input_media = $('#media'); // Added ID for media input
...
function initializeWebSocket() {
    ...
    socket.onopen = async function (e) {
        ...
        send_message_form.on('submit', function (e) {
            ...
            let mediaFile = input_media[0].files[0]; // Updated ID for media input
            let send_to = get_active_other_user_id();
            let thread_id = get_active_thread_id();

            let data = {
                'message': message,
                'media': null, // Initialize media field as null
                'sent_by': USER_ID,
                'send_to': send_to,
                'thread_id': thread_id
            };
            ...
            if (mediaFile) {
                // If a media file is selected, read it as a base64-encoded string
                const reader = new FileReader();
                reader.onload = function (event) {
                    data.media = event.target.result;
                    socket.send(JSON.stringify(data));
                };
                reader.readAsDataURL(mediaFile);
            } else {
                // If no media file is selected, send the data as is
                socket.send(JSON.stringify(data));
            }
    ...
    socket.onmessage = async function (e) {
        ...
        newMessage(message, sent_by_id, thread_id, media);
    };
    ...
    socket.onclose = async function (e) {
        if (e.wasClean) {
            console.log('WebSocket connection closed cleanly.', e);
        } else {
            console.error('WebSocket connection closed unexpectedly.', e);
            // Handle the unexpected closure and attempt to reconnect
...
function newMessage(message, sent_by_id, thread_id, media) {
    if ($.trim(message) === '') {
        return false;
    }
    let message_element;
    ...
    if (sent_by_id == USER_ID) {
        if (media) {
            // Display media sent by the user
            message_element = `
                <div class="d-flex mb-4 replied">
                    <div class="msg_cotainer_send">
                        <a href="${media}" target="_blank"><img src="${media}" class="media_file"></a>
                        <span class="msg_time_send">8:55 AM, Today</span>
                    </div>
                    <div class="img_cont_msg">
                        <img src="data:image/jpeg;base64,/9j/4" class="rounded-circle user_img_msg">
                    </div>
                </div>
            `;
        } else {
            // Display text message sent by the user
            message_element = `
                <div class="d-flex mb-4 replied">
                    <div class="msg_cotainer_send">
                        ${message}
        ...
        // Display received messages (both text and media)
        message_element = `
...
// Listen for changes in the media input field
let inputmedia = $('#media');
inputmedia.on('change', function() {
    console.log('Media input change event triggered');

    const mediaFile = this.files[0];
    console.log('Selected media file:', mediaFile);

    const mediaPreviewContainer = $('#media-preview-container');
    console.log('Media preview container:', mediaPreviewContainer);

    // Clear any previous previews
    mediaPreviewContainer.empty();

    if (mediaFile) {
        // Create a preview element based on the file type (e.g., image or video)
        const fileType = mediaFile.type.split('/')[0];
        if (fileType === 'image') {
            // For image files, create an <img> element
            const img = $('<img>');
            img.attr('src', URL.createObjectURL(mediaFile));
            img.attr('alt', 'Media Preview');
            mediaPreviewContainer.append(img);
        } else if (fileType === 'video') {
            // For video files, create a <video> element
            const video = $('<video controls>');
            video.attr('src', URL.createObjectURL(mediaFile));
            video.attr('alt', 'Media Preview');
            mediaPreviewContainer.append(video);
        } else {
            // Handle other types of media here (e.g., audio)
            const unsupportedMessage = $('<p>').text('Unsupported media type');
            mediaPreviewContainer.append(unsupportedMessage);
        }
    }
    console.log('Debugging output - after updating media preview');
});

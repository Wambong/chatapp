...
let mediaSelected = false; // Variable to track if media is selected
...
function displayMediaPreview(file) {
    ...
    if (file && file.type.startsWith('image/')) {
        ...
        reader.onload = function (event) {
            const mediaPreview = `
              <div class="media-preview-container">
                    <img src="${event.target.result}" class="media_preview" style="max-width: 250px; max-height: 250px; width: auto; height: auto; border-radius: 15px;">
                    <button class="btn btn-secondary reset-media-button" type="button">
                        <i class="fas fa-times"></i>
                    </button>
                </div> `;

            mediaPreviewContainer.html(mediaPreview);
             mediaSelected = true;
          // Add an event listener to the reset button
            mediaPreviewContainer.find('.reset-media-button').click(function () {
        ...
        mediaPreviewContainer.html(''); // Clear the preview if no image is selected
        mediaSelected = false;
    }
}
input_media.on('change', function () {
    const mediaFile = this.files[0];
    displayMediaPreview(mediaFile);
});


let mediaFile = input_media[0].files[0];
displayMediaPreview(mediaFile); // Display media preview
...
function initializeWebSocket() {
    ...
    socket.onopen = async function (e) {
        ...
        send_message_form.on('submit', function (e) {
            ...
            // Clear the media preview only if media was selected
            if (mediaSelected) {
                    $('#media-preview-container').html('');
                    mediaSelected = false; // Reset the variable
            }
            $(this)[0].reset();
        });
    };
...
function newMessage(message, sent_by_id, thread_id, media) {
    ...
    if (sent_by_id == USER_ID) {
        ...
        if (media) {
            // Display media received from other users
            message_element = `
